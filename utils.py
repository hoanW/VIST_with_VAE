...
def bb_matching(bbox_pro: Tensor, bbox_mask_rcnn: Tensor, last_id: Tensor):
  ...
  """
  assert (len(last_id) == len(bbox_pro)),"len(last_id) should be equal to len(bbox_pro), missing bbox_pro"
  ...
  for i in range(len(bbox_pro)):
      for j in range(len(bbox_mask_rcnn)):
        matching_score = bb_IoU(bbox_pro[i], bbox_mask_rcnn[j])  #0: mismatch -> 1: perfect match
  ...
  if len(union_bb_list) == 0:
     union_bb_list = [Variable(torch.zeros(4,), requires_grad=False).to(device)]
     
  return torch.stack(union_bb_list).to(device), torch.tensor(union_id).to(device)

def batch_bb_matching(bbox_pro: List[Tensor], bbox_mask_rcnn: List[Tensor], last_id: List[Tensor]):
  batch_bbs = []
  batch_id = []
...
def pro_postprocess(pred, target):
    ...
    if torch.nonzero(pro_positive_roi_bool).size():
        ...
        if len(pro_target_bbox) != 0:
           pro_deltas_target =  box_refinement(pro_positive_rois, pro_target_bbox).detach().to(device)
        else:
    ...
    if pro_positive_count > 0 and pro_negative_count > 0:
        ...
        pro_deltas_pred_bbox = torch.cat((pro_deltas_positive_pred, pro_deltas_negative_pred), dim=0)
        zeros = Variable(torch.zeros(pro_negative_count,4), requires_grad=False).int().to(device)
        ...
        pro_deltas_pred_bbox = Variable(torch.FloatTensor(), requires_grad=False).to(device)
        pro_deltas_target_bbox = Variable(torch.FloatTensor(), requires_grad=False).to(device)
...
def aug_postprocess(pred, target):
    ...
    target_class.squeeze(0), target_bbox.squeeze(0), target_masks.squeeze(0)
    ...
    if torch.nonzero(aug_positive_roi_bool).size():
        ...
        if len(aug_target_bbox) != 0:
            aug_deltas_target = box_refinement(aug_positive_rois, aug_target_bbox).detach().to(device)
        else:
    ...
    if aug_positive_count > 0 and aug_negative_count > 0:
        ...
        aug_deltas_pred_bbox = torch.cat((aug_deltas_positive_pred, aug_deltas_negative_pred), dim=0)
        
        pred_logits_class = torch.cat((positive_pred_logits_class, aug_pred_negative_logits_class), dim=0)
        pred_masks = torch.cat((positive_pred_masks, aug_pred_negative_masks), dim=0)

        zeros = Variable(torch.zeros(aug_negative_count,4), requires_grad=False).int().to(device)
        aug_deltas_target_bbox =  torch.cat([aug_deltas_target, zeros], dim=0)
        roi_target_bbox = torch.cat([aug_target_bbox, zeros], dim = 0)
        
        zeros = Variable(torch.zeros(aug_negative_count), requires_grad=False).int().to(device)
        target_class = torch.cat([target_class, zeros])
        zeros = Variable(torch.zeros(aug_negative_count,768,1280), requires_grad=False).int().to(device)
        ...
        aug_deltas_pred_bbox = Variable(torch.FloatTensor(), requires_grad=False).to(device)
        ...
        aug_deltas_target_bbox = Variable(torch.FloatTensor(), requires_grad=False).to(device)
    ...
    return aug_deltas_pred_bbox, pred_logits_class, pred_masks, \
           aug_deltas_target_bbox, target_class, target_masks, roi_target_bbox
...
def prediction(sample, model):
        ...
        boxes = [[(round(max(i[0] *1728/1280, 0),2), round(max(i[1] *1296/768, 0),2)), (round(max(i[2]*1728/1280,0),2), round(max(i[3] *1296/768,0),2))] for i in list(results[2]["boxes"].cpu().numpy())]
...
def unmold_mask(mask, bbox, image_shape=data_params["FRAME_SIZE"]):
    ...
    mask1 = np.array(Image.fromarray(mask).resize((x2-x1, y2-y1))).astype(np.float32) 
