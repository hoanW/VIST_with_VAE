...
def bbox_detector(img):
    ...
    idx = [
        pred_score.index(x)
        for x in pred_score
        if x > model_params["DETECTION_THRESHOLD"]
    ]
...
class Modified_VAE(BaseVAE):
    def __init__(
        ...
        **kwargs
        ...
        for fc_dim in fc_dims:
            fc_modules.append(
                nn.Sequential(nn.Linear(fc_in_dim, fc_dim), nn.LeakyReLU())
            )
            fc_in_dim = fc_dim
        ...
        self.de_layer1 = self._make_transpose(
            transblock, 512, layers[-1], stride=2
        )  # (512, 6, 10)
        self.de_layer2 = self._make_transpose(
            transblock, 512, layers[-2], stride=2
        )  # (512, 12, 20)
        self.de_layer3 = self._make_transpose(
            transblock, 512, layers[-3], stride=2
        )  # (512, 24, 40)
        self.de_layer4 = self._make_transpose(
            transblock, 256, layers[-4], stride=2
        )  # (256, 48, 80)
        self.de_layer5 = self._make_transpose(
            transblock, 128, 1, stride=2
        )  # (128, 96, 160)
        self.de_layer6 = self._make_transpose(
            transblock, 64, 1, stride=2
        )  # (64, 192, 320)
        self.de_layer7 = self._make_transpose(
            transblock, 64, 1, stride=2
        )  # (64, 384, 640)
        ...
        self.final_layer = nn.Sequential(
            nn.ConvTranspose2d(
                64, 8, kernel_size=3, stride=2, padding=1, output_padding=1
            ),
            nn.BatchNorm2d(8),
            nn.LeakyReLU(),
            nn.Conv2d(8, out_channels=3, kernel_size=3, padding=1),
            nn.Sigmoid(),
        )
        ...
        self.pro_SPATIAL_SCALE = (
            1.0 / 8.0
        )  # pooling from DevCon5 (Conv x2) has size of DeConv5:  (1, 128, 96, 160)
        self.pro_roi_align = RoIAlign(self.pro_POOLING_SIZE, self.pro_SPATIAL_SCALE, 2)

        pro_representation_size = (
            128 * self.pro_POOLING_SIZE * self.pro_POOLING_SIZE
        )  # 64 is channel_size of pooled image (Deconv4)
        ...
        self.aug_SPATIAL_SCALE = (
            1.0 / 16.0
        )  # pooling from Conv x3 has size of (1, 256, 48, 80)
        self.aug_roi_align = RoIAlign(self.aug_POOLING_SIZE, self.aug_SPATIAL_SCALE, 2)

        aug_representation_size = (
            256 * self.aug_POOLING_SIZE * self.aug_POOLING_SIZE
        )  # 512 is channel_size of pooled image (x4)
        ...
        self.mask_fcn1 = nn.Conv2d(
            256, 256, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1)
        )
        self.mask_fcn2 = nn.Conv2d(
            256, 256, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1)
        )
        self.mask_fcn3 = nn.Conv2d(
            256, 256, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1)
        )
        self.mask_fcn4 = nn.Conv2d(
            256, 256, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1)
        )
        self.mask_relu = nn.ReLU(inplace=True)
        self.mask_conv5 = nn.ConvTranspose2d(
            256, 256, kernel_size=(2, 2), stride=(2, 2)
        )
        self.mask_fcn_logits = nn.Conv2d(256, 5, kernel_size=(1, 1), stride=(1, 1))
        self.mask_sigmoid = nn.Sigmoid()

        # Roi for loss
        self.loss_roi_align = RoIAlign(
            self.aug_POOLING_SIZE * 2, 1, 2
        )  # pool form original mask (1, 1280, 768)
        self.relu = nn.ReLU(inplace=True)
    ...
    def de_proposal(
        self, z: Tensor, x: List[Tensor], last_bboxes: List[Tensor]
    ) -> Tensor:
        """
        Maps the given latent codes
        onto the image space.
    ...
    def loss_function(self, pred, target, mu, log_var) -> dict:
        """
        ...
        if target[1].size():
            pro_pred, pro_target = pro_postprocess(
                [pro_pred_bbox, pro_rois], target_bbox
            )
            ...
            (
                ...
                [aug_pred_bbox, pred_prob_class, pred_masks, aug_rois],
                [target_bbox, target_class, target_masks],
            )
            ...
            if len(target_class) != 0:
                ...
                roi_target_masks = self.loss_roi_align(
                    target_masks.unsqueeze(1).float(), roi_bbox
                )  # unsqueeze(1) to add one channel to mask

                masks_loss = nnF.binary_cross_entropy_with_logits(
                    pred_masks, roi_target_masks.squeeze(1)
                )

            else:
                class_loss = Variable(torch.FloatTensor([0]), requires_grad=False).to(
                    device
                )
                masks_loss = Variable(torch.FloatTensor([0]), requires_grad=False).to(
                    device
                )

            aug_loss = (
                self.params["box_weight"] * aug_boxes_loss
                + class_loss
                + self.params["mask_weight"] * masks_loss
            )

        else:
            pro_boxes_loss = Variable(torch.FloatTensor([0]), requires_grad=False).to(
                device
            )
        ...
        loss = (
            beta * recons_loss
            + alpha * kld_loss
            + self.params["mask_weight"] * pro_boxes_loss
            + aug_loss
        )
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log(
            "Total loss/Train step", train_loss["loss"], on_step=True, on_epoch=True
        )
        self.log(
            "Recons loss/Train step",
            train_loss["recons_loss"],
            on_step=False,
            on_epoch=True,
        )
        self.log(
            "Proposal boxes loss/Train step",
            train_loss["pro_loss"],
            on_step=False,
            on_epoch=True,
        )
        self.log(
            "Augment branch loss/Train step",
            train_loss["aug_loss"],
            on_step=False,
            on_epoch=True,
        )
    ...
    def validation_step(self, batch, batch_idx):
        ...
        validation_loss = self.loss_function(
            pred, target, mu=results[3], log_var=results[4]
        )

        self.log(
            "Total loss/Validation epoch",
