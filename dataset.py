...
class Mask_RCNN_Dataset(object):
    def __init__(self, images, annotations, transforms=None):
        ...
        self.csvdata.rename(
            columns={"region_shape_attributes": "xpoints"}, inplace=True
        )

        self.classdict = data_params["CLASS_DICT"]
        # for splitting attribut values, insert the new columns
        self.csvdata["track"] = pd.Series(
            np.random.randn(self.csvdata.shape[0]), index=self.csvdata.index
        )
        self.csvdata.insert(
            6,
            "ypoints",
            pd.Series(np.random.randn(self.csvdata.shape[0]), index=self.csvdata.index),
        )
        ...
        for i in range(self.csvdata.shape[0]):
            ...
            if int(self.csvdata.loc[i, "region_count"]) > 0:
                ...
                # insert image ids
                self.csvdata.loc[i, "image_id"] = int(
                    imgid_dict[self.csvdata.loc[i, "filename"]]
                )
...
class AnimalsDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage):
        self.training_data = VAE_Dataset(
            data_params["PATH_TRAINING_SET"],
            data_params["PATH_ANNO"],
            transforms=get_transform(train=True),
        )
        self.test_data = VAE_Dataset(
            data_params["PATH_TEST_SET"],
            data_params["PATH_ANNO"],
            transforms=get_transform(train=False),
        )

    def train_dataloader(self):
        return DataLoader(self.training_data, batch_size=1, shuffle=True, num_workers=4)
...
class Eval_Dataset(Dataset):
    def __init__(self, images, annotations, transforms, video_name):
        ...
        # get imgs path in an array
        self.imgs = list(
            sorted(i for i in os.listdir(self.imgpath) if i.startswith(self.video_name))
        )
        # get len of this array
        self.len = len(self.imgs)
