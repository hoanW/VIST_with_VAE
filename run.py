...
data_params = {
    "PATH_TRAINING_SET": "./WildlifeCrossingDataset_Training",  # path to frame folder
...
model_params = {
    "DETECTION_THRESHOLD": 0.95,  # detecion threshold score
...
pre_trained_model = {
    "PATH_MASK_RCNN": "../TrainedParameters/MaskRCNN/MaskRCNN_WildlifeCrossing.pth",
...
if __name__ == "__main__":
    ...
    mask_rcnn_model = get_instance_segmentation_model(data_params["NUM_CLASSES"]).to(
        device
    )
    mask_rcnn_model.load_state_dict(
        torch.load(pre_trained_model["PATH_MASK_RCNN"], map_location=device)
    )
    mask_rcnn_model.eval()

    # Get VAE model
    vae_model = Modified_VAE(
        in_channels=7,
